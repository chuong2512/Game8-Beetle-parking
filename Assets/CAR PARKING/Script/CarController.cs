    ...
    public IEnumerator Action()
    {
        ...
        while (currentPos < listOfPoints.Count)
        {
            ...
            while ((Vector2.Distance(transform.position, posTarget) > stopDistance)
                && (Vector2.Distance(transform.position, posTarget) < Vector2.Distance(transform.position, listOfPoints[currentPos + 1])))     //wait until the car reach to the destination
            {
                yield return null;
            }
